...
export const DOCS_TOOLS: readonly DocsTool[] = [
  { id: "copyKeyframes", name: "Copy Keys", cat: "motion", preview: "sheet", ko: "클릭: 키프레임 복사 · Alt+클릭: 이징 복사 (속도를 정규화해 담으므로 이동거리·길이가 달라도 느낌이 그대로 옮겨간다) · Shift+클릭: 이즈 클립보드 비우기", en: "Click: copy keyframes · Alt+click: copy easing (speed is normalised, so the feel carries over even when distance or duration differs) · Shift+click: clear the ease clipboard" },
  { id: "pasteKeyframes", name: "Paste Keys", cat: "motion", preview: "sheet", ko: "클릭: 플레이헤드에 키프레임 붙여넣기 · Shift+클릭: 역순으로 붙여넣기 · Alt+클릭: 이징 붙여넣기 (키의 시간·값은 그대로, 이징만 바뀐다) · Alt+Shift+클릭: 이징 붙여넣기 + 통과 속도 보존 (원본에서 in/out 속도가 같던 키만) · 우클릭 메뉴: 값 붙여넣기", en: "Click: paste keyframes at the playhead · Shift+click: paste reversed · Alt+click: paste easing (key times and values stay put, only the easing changes) · Alt+Shift+click: paste easing and preserve pass-through (only for keys whose in/out speeds matched in the source) · Right-click menu: paste values" },
  ...
  { id: "startMarker", name: "Start Marker", cat: "motion", preview: "sheet", ko: "프리컴프를 열지 않고 안쪽 내용의 시작 시각을 바깥 마커로 조절 — 마커를 끌면 시작이 따라온다 (다시 누르면 해제)", en: "Set when a precomp's inner content starts, from outside it — drag the marker and the start follows (press again to remove)" },
  ...
  { id: "depthPass", name: "Depth Pass", cat: "layer", preview: "svg", ko: "선택 레이어에서 뎁스 맵 생성 → 가이드 레이어 (렌즈 블러·피사계 심도용)", en: "Build a depth map from the selected layers → guide layer (for lens blur / depth of field)" },
  { id: "randomFill", name: "Multi Fill", cat: "layer", preview: "sheet", ko: "선택 레이어에 팔레트 색 배정 — 다이얼로그에서 무작위(인접 회피) 또는 순서대로(타임라인 아래→위 순환)를 고름 (셰이프 Fill/텍스트/그 외는 Fill 이펙트)", en: "Assign palette colors to the selected layers — pick random (avoids adjacent repeats) or in order (bottom to top in the timeline, cycling) in the dialog (shape Fill / text / a Fill effect on the rest)" },
  ...
  { id: "organizeProject", name: "Organize Project", cat: "comp", preview: "sheet", ko: "프로젝트 패널을 스키마 폴더로 자동 정리 — 렌더/컴프/소스/솔리드/레퍼런스 (적용 전 계획 미리보기)", en: "Sort the Project panel into schema folders — render/comp/source/solids/reference (plan preview before applying)" },
  ...
  { id: "patternLab", name: "Pattern Lab", cat: "shape", preview: "sheet", ko: "배경 패턴 생성 — 도트 격자·해치·체커·하프톤·트뤼셰·동심원·그레인 그라디언트·아스키 필드 8종 + 간격·회전·색. 셰이프(벡터) 또는 PNG(픽셀)로 출력하며 익스프레션·이펙트를 쓰지 않아 재생이 무거워지지 않습니다", en: "Generate a background pattern — 8 kinds: dot grid, hatch, checker, halftone fade, truchet, concentric, grain gradient, ASCII field, with spacing, rotation and color. Outputs as shapes (vector) or PNG (pixels), using no expressions and no effects so playback stays light" },
